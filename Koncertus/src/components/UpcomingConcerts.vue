<template>
    <h1>Upcoming concerts</h1>
    <!-- 
        For importing icons:
        <font-awesome-icon icon="fa-solid fa-calendar" />
        <font-awesome-icon icon="fa-brands fa-facebook" /> 
      -->
    <div class="concertCardContainer">
        <div class="concertCard">
            <div class="concertCardImage"></div>
            <div class="concertCardText">
                <p class="concertCardTextTitle">Disturbed</p>
                <p><font-awesome-icon icon="fa-solid fa-calendar" class="paddingRightCardIcon"/> 05.02.2023.</p>
                <p><font-awesome-icon icon="fa-solid fa-clock" class="paddingRightCardIcon"/> 20:15</p>
                <div class="sameLine">
                    <p><font-awesome-icon icon="fa-solid fa-location-dot" class="paddingRightCardIcon"/> Arena Zagreb</p>
                    <p class="alignRight">20 EUR</p>
                </div>
            </div>
        </div>
        <div class="concertCard">
            <div class="concertCardImage"></div>
            <div class="concertCardText">
                <p class="concertCardTextTitle">Disturbed</p>
                <p><font-awesome-icon icon="fa-solid fa-calendar" class="paddingRightCardIcon"/> 05.02.2023.</p>
                <p><font-awesome-icon icon="fa-solid fa-clock" class="paddingRightCardIcon"/> 20:15</p>
                <div class="sameLine">
                    <p><font-awesome-icon icon="fa-solid fa-location-dot" class="paddingRightCardIcon"/> Arena Zagreb</p>
                    <p class="alignRight">20 EUR</p>
                </div>
            </div>
        </div>
        <div class="concertCard">
            <div class="concertCardImage"></div>
            <div class="concertCardText">
                <p class="concertCardTextTitle">Disturbed</p>
                <p><font-awesome-icon icon="fa-solid fa-calendar" class="paddingRightCardIcon"/> 05.02.2023.</p>
                <p><font-awesome-icon icon="fa-solid fa-clock" class="paddingRightCardIcon"/> 20:15</p>
                <div class="sameLine">
                    <p><font-awesome-icon icon="fa-solid fa-location-dot" class="paddingRightCardIcon"/> Arena Zagreb</p>
                    <p class="alignRight">20 EUR</p>
                </div>
            </div>
        </div>
        <div class="concertCard">
            <div class="concertCardImage"></div>
            <div class="concertCardText">
                <p class="concertCardTextTitle">Disturbed</p>
                <p><font-awesome-icon icon="fa-solid fa-calendar" class="paddingRightCardIcon"/> 05.02.2023.</p>
                <p><font-awesome-icon icon="fa-solid fa-clock" class="paddingRightCardIcon"/> 20:15</p>
                <div class="sameLine">
                    <p><font-awesome-icon icon="fa-solid fa-location-dot" class="paddingRightCardIcon"/> Arena Zagreb</p>
                    <p>20 EUR</p>
                </div>
            </div>
        </div>
    </div>
</template>